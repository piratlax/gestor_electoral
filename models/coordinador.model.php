...
class ModeloCoordinadores{
    ...
    static public function mdlMostrarCoordinadores($tabla, $item, $valor){
        if($item != null){
			...
			$stmt = Conexion::conectar()->prepare("SELECT * from usuarios as u INNER JOIN personas as p ON u.persona_clave = p.persona_clave INNER JOIN organizaciones ON u.organizaciones_id = organizaciones.id WHERE perfil='Coordinador';");
