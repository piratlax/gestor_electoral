...
class ModeloUsuarios{
  ...
  static public function mdlMostrarUsuarios($tabla, $campo, $valor){
   if($campo != null){
			...
			$stmt = Conexion::conectar()->prepare("SELECT p.id, p.persona_clave as persona_clave, concat(p.nombre, ' ',p.paterno,' ',p.materno) as nombre, concat(p.direccion,' ',p.colonia) as direccion, concat(p.localidad,' Seccion:',p.seccion) as localidad, u.usuario as usuario, p.foto as foto, u.perfil as perfil, u.estado as estado, u.ultimo_login as login,  p.inef as inef, p.inet as inet, p.ocupacion as ocupacion, u.password as password, u.organizaciones_id as organizaciones_id FROM usuarios as u INNER JOIN personas as p ON u.persona_clave=p.persona_clave WHERE u.$campo = :$campo; ");
