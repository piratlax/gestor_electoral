	...
	static public function mdlMostrarPromovidos($tabla, $item, $valor)
	{
		if ($item != null) {
			...
			if ($_SESSION["perfil"] == 'Administrador') {
				$stmt = Conexion::conectar()->prepare("SELECT p.id as id, concat(persona.nombre,' ',persona.paterno,' ',persona.materno) as nombre, persona.direccion as direccion, persona.colonia as colonia, secciones.numero as seccion, persona.celular as celular, persona.foto as foto, concat (e.nombre, ' ',e.paterno,' ',e.materno) as enlace, o.organizacion as organizacion  FROM promovidos as p inner join personas as persona on p.persona_clave = persona.persona_clave inner join personas as e on e.persona_clave=p.enlace_clave INNER JOIN usuarios as u ON e.persona_clave=u.persona_clave  INNER JOIN secciones ON persona.idseccion=secciones.id INNER JOIN organizaciones as o ON u.organizaciones_id=o.id");
			} else {
